...
while [[ "$install_key" == 'yes' ]]; do


    if ! is_valid_internet >/dev/null 2>&1; then
        echo "NO INTERNET CONNECTION DETECTED.  "
        echo ""
        install_key=$(ask_yes_no "Do you want to connect to internet and try again?")
    else
        ...
        if [[ "$?" -lt 2 ]]; then
            ...
            key_file=''
            echo "NO VALID GITHUB SSH KEY FOUND"
            echo ""
            install_key=$(ask_yes_no "Do you want to create a new GitHub SSH Key?")
            ...
            if [ "$install_key" == "yes" ]; then
                ...
                echo "You can rerun this script if you want to setup at a later time"
                echo ""
            fi
        fi
