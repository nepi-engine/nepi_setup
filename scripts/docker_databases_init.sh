...
if [[ ! -f "$SYSTEM_CONFIG_LOAD_FILE" ]]; then
    ...
    if [[ "$?" -eq 1 ]]; then
        ...
        if [[ "$avail_space_gb" -lt "$req_space_gb" ]]; then
            ...
            echo ""
            echo "#################################"
            echo "Initializing NEPI Databases Folders"
            echo "#################################"
            echo ""
            ...
            databases_latest_link='https://www.dropbox.com/scl/fi/ev39v337vawtuxvkl061y/nepi_databases-latest.zip?rlkey=urohkkln0u3xd0fwg87zetsyp&st=ghiu73hv&dl=0'
            ...
            if [[ ! -f ${databases_latest_zip} ]]; then
                ...
                if [[ "$?" -ne 0 ]]; then
                    echo ""
                    echo "Failed to download NEPI Storage from link: ${databases_latest_link}"
                    echo ""
            ...
            if [[ -f ${databases_latest_zip} ]]; then
                ...
                if [ $? -eq 0 ]; then
                    ...
                    echo ""
                    echo "Failed to unzip NEPI Storage file: ${databases_latest_zip}"
                    echo ""
                    #sudo rm ${databases_latest_zip} > /dev/null 2>&1
                fi
            else
                echo ""
                echo "Failed to find NEPI Storage file: ${databases_latest_zip}"
                echo ""
            fi
            ...
            if [[ "$success_databases" -eq 0 ]]; then
                echo "NEPI Databases Setup Failed"
                echo ""
            else
                echo ""
                echo "NEPI Databases Setup Succeeded"
            fi


            if [[ "$success_databases" -eq 1 && "$success_image" -eq 1 ]]; then
                echo ""
                echo "########################"
                echo "NEPI Docker Databases Init Complete"
                echo "########################"
                echo ""
